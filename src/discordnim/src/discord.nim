...
proc updateStreamingStatus*(s: Shard, idle: int = 0, game: string,
    url: string = "", status: string = "online") {.async, gcsafe.} =
...
proc reconnect(s: Shard) {.async, gcsafe.} =
  ...
  try:
    s.connection = await newAsyncWebsocketClient("gateway.discord.gg", Port 443,
        "/"&GATEWAYVERSION, true)
  except:
...
proc startSession*(s: Shard) {.async, gcsafe.} =
  ...
  try:
    ...
    echo "Something happened in the socket listening :: ",
        getCurrentExceptionMsg()
