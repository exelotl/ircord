...
import httpclient, asyncdispatch, json, tables, re,
    strutils, ospaths, mimetypes, uri, options
import objects except `%`
...
proc request(s: Shard,
  ...
  let client = newAsyncHttpClient("DiscordBot (https://github.com/Krognol/discordnim, v" &
      VERSION & ")")
...
proc doreq(s: Shard, meth, endpoint, payload: string = "",
    xheaders: HttpHeaders = nil, mpd: MultipartData = nil): Future[JsonNode] {.
    gcsafe, async.} =
  let res = await s.request(endpoint, meth, endpoint, "application/json",
      payload, 0, xheaders = xheaders)
...
proc channelEdit*(s: Shard, channelid: string, params: ChannelParams,
    reason: string = ""): Future[Guild] {.gcsafe, async.} =
...
proc channelMessages*(s: Shard, channelid: string, before, after,
    around: string, limit: int): Future[seq[Message]] {.gcsafe, async.} =
...
proc channelMessage*(s: Shard, channelid, messageid: string): Future[Message] {.
    gcsafe, async, inline.} =
  ## Returns a message from a channel
  result = (await doreq(s, "GET", endpointChannelMessage(channelid,
      messageid))).newMessage

proc channelMessageSend*(s: Shard, channelid, message: string): Future[
    Message] {.gcsafe, async.} =
...
proc channelMessageSendEmbed*(s: Shard, channelid: string,
    embed: Embed): Future[Message] {.gcsafe, async, inline.} =
...
proc channelMessageSendTTS*(s: Shard, channelid, message: string): Future[
    Message] {.gcsafe, async, inline.} =
...
proc channelFileSendWithMessage*(s: Shard, channelid, name,
    message: string): Future[Message] {.gcsafe, async.} =
...
proc channelFileSendWithMessage*(s: Shard, channelid, name, fbody,
    message: string): Future[Message] {.gcsafe, async.} =
...
proc channelFileSend*(s: Shard, channelid, fname: string): Future[Message] {.
    gcsafe, inline, async.} =
  ## Sends a file to a channel
  result = await s.channelFileSendWithMessage(channelid, fname, "")

proc channelFileSend*(s: Shard, channelid, fname, fbody: string): Future[
    Message] {.gcsafe, inline, async.} =
  ## Sends the contents of a file as a file to a channel.
  result = await s.channelFileSendWithMessage(channelid, fname, fbody, "")

proc channelMessageReactionAdd*(s: Shard, channelid, messageid,
    emojiid: string) {.gcsafe, inline, async.} =
  ## Adds a reaction to a message
  asyncCheck doreq(s, "PUT", endpointMessageReactions(channelid, messageid, emojiid))

proc messageDeleteOwnReaction*(s: Shard, channelid, messageid,
    emojiid: string) {.gcsafe, inline, async.} =
  ## Deletes your own reaction to a message
  asyncCheck doreq(s, "DELETE", endpointOwnReactions(channelid, messageid, emojiid))

proc messageDeleteReaction*(s: Shard, channelid, messageid, emojiid,
    userid: string) {.gcsafe, inline, async.} =
  ## Deletes a reaction from a user from a message
  asyncCheck doreq(s, "DELETE", endpointMessageUserReaction(channelid,
      messageid, emojiid, userid))

proc messageGetReactions*(s: Shard, channelid, messageid,
    emojiid: string): Future[seq[User]] {.gcsafe, inline, async.} =
  ## Gets a message's reactions
  let node = (await doreq(s, "GET", endpointMessageReactions(channelid,
      messageid, emojiid)))
  result = newSeq[User](node.elems.len)
  for i, n in node.elems:
    result[i] = newUser(n)

proc messageDeleteAllReactions*(s: Shard, channelid, messageid: string) {.
    gcsafe, inline, async.} =
  ## Deletes all reactions on a message
  asyncCheck doreq(s, "DELETE", endpointReactions(channelid, messageid))

proc channelMessageEdit*(s: Shard, channelid, messageid,
    content: string): Future[Message] {.gcsafe, inline, async.} =
  ## Edits a message's contents
  result = (await doreq(s, "PATCH", endpointChannelMessage(channelid,
      messageid), $(%*{"content": content}))).newMessage

proc channelMessageDelete*(s: Shard, channelid, messageid: string,
...
proc channelMessagesDeleteBulk*(s: Shard, channelid: string, messages: seq[
  ...
  asyncCheck doreq(s, "DELETE", endpointBulkDelete(channelid), $(%*{
      "messages": messages}))

proc channelEditPermissions*(s: Shard, channelid: string, overwrite: Overwrite,
    reason: string = "") {.gcsafe, async.} =
...
proc channelCreateInvite*(
  ...
  let payload = %*{"max_age": max_age, "max_uses": max_uses, "temp": temp,
      "unique": unique}
  let xh = if reason != "": newHttpHeaders({
      "X-Audit-Log-Reason": reason}) else: nil
...
proc channelPinnedMessages*(s: Shard, channel: string): Future[seq[Message]] {.
    gcsafe, inline, async.} =
...
proc channelDeletePinnedMessage*(s: Shard, channel, message: string) {.gcsafe,
    inline, async.} =
...
# This might work?
proc groupDMCreate*(s: Shard, accesstokens: seq[string], nicks: seq[
    AddGroupDMUser]): Future[Channel] {.gcsafe, async, inline.} =
...
proc groupDMAddUser*(s: Shard, channelid, userid, access_token, nick: string) {.
    gcsafe, async, inline.} =
...
proc newPartialChannel*(name: string, typ: int = 0): PartialChannel {.
    inline.} = PartialChannel(name: name, `type`: typ)
...
proc guildEdit*(s: Shard, guild: string, settings: GuildParams,
    reason: string = ""): Future[Guild] {.gcsafe, async.} =
...
proc guildChannelCreate*(
  ...
  var payload = %*{"name": channelname, "parent_id": parentId, "voice": voice,
      "rate_limit_per_user": rateLimit, "nsfw": nsfw}
...
proc guildChannelPositionEdit*(s: Shard, guild, channel: string, position: int,
    reason: string = ""): Future[seq[Channel]] {.gcsafe, async.} =
...
proc guildMembers*(s: Shard, guild: string, limit, after: int): Future[seq[
    GuildMember]] {.gcsafe, async.} =
...
proc guildMember*(s: Shard, guild, userid: string): Future[GuildMember] {.
    gcsafe, async.} =
  ## Returns a guild member with the userid
  result = (await doreq(s, "GET", endpointGuildMember(guild,
      userid))).newGuildMember

proc guildAddMember*(s: Shard, guild, userid, accesstoken: string): Future[
    GuildMember] {.gcsafe, async.} =
...
proc guildMemberRolesEdit*(s: Shard, guild, userid: string, roles: seq[
    string]) {.gcsafe, async.} =
  ## Edits a guild member's roles
  asyncCheck doreq(s, "PATCH", endpointGuildMember(guild, userid), $(%*{
      "roles": roles}))

proc guildMemberSetNickname*(s: Shard, guild, userid, nick: string,
    reason: string = "") {.gcsafe, async.} =
  ## Sets the nickname of a member
  asyncCheck doreq(s, "PATCH", endpointGuildMember(guild, userid), $(%*{"nick": nick}))

proc guildMemberMute*(s: Shard, guild, userid: string, mute: bool,
    reason: string = "") {.gcsafe, async.} =
...
proc guildUserBan*(s: Shard, guild, userid: string, reason: string = "") {.
    gcsafe, async.} =
...
proc guildCreateRole*(s: Shard, guild: string, reason: string = ""): Future[
    Role] {.gcsafe, async.} =
...
proc guildEditRolePosition*(s: Shard, guild: string, roles: seq[Role],
    reason: string = ""): Future[seq[Role]] {.gcsafe, async.} =
...
proc guildEditRole*(
            ...
            {.gcsafe, async.} =
  ## Edits a role
  let payload = %*{"name": name, "permissions": permissions, "color": color,
...
proc guildPruneBegin*(s: Shard, guild: string, days: int,
    reason: string = ""): Future[int] {.gcsafe, async.} =
...
proc guildInvites*(s: Shard, guild: string): Future[seq[Invite]] {.gcsafe,
    inline, async.} =
...
proc guildIntegrations*(s: Shard, guild: string): Future[seq[Integration]] {.
    gcsafe, inline, async.} =
...
proc guildIntegrationEdit*(s: Shard, guild, integrationid: string, behaviour,
    grace: int, emotes: bool) {.gcsafe, async.} =
  ## Edits a guild integration
  let payload = %*{"expire_behavior": behaviour, "expire_grace_period": grace,
      "enable_emoticons": emotes}
  asyncCheck doreq(s, "PATCH", endpointGuildIntegration(guild, integrationid), $payload)

proc guildIntegrationDelete*(s: Shard, guild, integration: string) {.gcsafe,
    inline, async.} =
  ## Deletes a guild Integration
  asyncCheck doreq(s, "DELETE", endpointGuildIntegration(guild, integration))

proc guildIntegrationSync*(s: Shard, guild, integration: string) {.gcsafe,
    inline, async.} =
...
proc guildEmbedEdit*(s: Shard, guild: string, enabled: bool,
    channel: string): Future[GuildEmbed] {.gcsafe, async.} =
  ## Edits a GuildEmbed
  let embed = GuildEmbed(enabled: enabled, channel_id: some(channel))
  result = (await doreq(s, "PATCH", endpointGuildEmbed(guild), $(
      %embed))).newGuildEmbed

proc guildEmojiCreate*(s: Shard, guild, name, image: string, roles: seq[
    string] = @[]): Future[Emoji] {.gcsafe, async.} =
  let payload = %*{
      "name": name,
      "image": image,
      "roles": roles
  }
  result = (await doreq(s, "POST", endpointGuildEmojis(guild),
      $payload)).newEmoji

proc guildEmojiUpdate*(s: Shard, guild, emoji, name: string, roles: seq[
    string] = @[]): Future[Emoji] {.gcsafe, async.} =
  ...
  result = (await doreq(s, "PATCH", endpointGuildEmoji(guild, emoji),
      $payload)).newEmoji
...
proc inviteDelete*(s: Shard, code: string, reason: string = ""): Future[
    Invite] {.gcsafe, async.} =
  ## Deletes an invite
  let xh = if reason != "": newHttpHeaders({
      "X-Audit-Log-Reason": reason}) else: nil
...
proc usernameEdit*(s: Shard, name: string): Future[User] {.gcsafe, inline, async.} =
  ...
  result = (await doreq(s, "PATCH", endpointCurrentUser(), $(%*{
...
proc privateChannelCreate*(s: Shard, recipient: string): Future[Channel] {.
    gcsafe, inline, async.} =
  ## Creates a new DM channel
  result = (await doreq(s, "POST", endpointDM(), $(%*{
      "recipient_id": recipient}))).newChannel
...
proc webhookCreate*(s: Shard, channel, name, avatar: string,
    reason: string = ""): Future[Webhook] {.gcsafe, async.} =
  ## Creates a webhook
...
proc guildWebhooks*(s: Shard, guild: string): Future[seq[Webhook]] {.gcsafe,
    inline, async.} =
...
proc getWebhookWithToken*(s: Shard, webhook, token: string): Future[Webhook] {.
    gcsafe, inline, async.} =
  ## Gets a webhook with a token
  result = (await doreq(s, "GET", endpointWebhookWithToken(webhook,
      token))).newWebhook

proc webhookEdit*(s: Shard, webhook, name, avatar: string,
    reason: string = ""): Future[Webhook] {.gcsafe, async.} =
  ## Edits a webhook
  let payload = %*{"name": name, "avatar": avatar}
  let xh = if reason != "": newHttpHeaders({
...
proc executeWebhook*(s: Shard, webhook, token: string,
    payload: WebhookParams) {.gcsafe, inline, async.} =
  ## Executes a webhook
...
proc defaultAvatar*(u: User): string =
  ...
  if get(u.avatar, "") == "":
    result = "https://cdn.discordapp.com/embed/avatars/$1.png" % [$(
        u.discriminator.parseInt mod 5)]
  else:
...
proc newWebhookParams*(content, username, avatarurl: string = "",
                       tts: bool = false, embeds: seq[Embed]): WebhookParams {.
                           gcsafe, inline.} =
