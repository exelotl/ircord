...
proc postCheck*(r: RateLimit, url: string, response: AsyncResponse): Future[
        bool] {.async, gcsafe.} =
  if response.headers.hasKey("X-RateLimit-Reset"): r.reset = response.headers[
          "X-RateLimit-Reset"].parseInt
  if response.headers.hasKey("X-RateLimit-Limit"): r.limit = response.headers[
          "X-RateLimit-Limit"].parseInt
  if response.headers.hasKey("X-RateLimit-Remaining"): r.remaining = response.headers[
...
proc postCheck*(r: RateLimits, url: string, response: AsyncResponse): Future[
        bool] {.async, gcsafe.} =
...
proc newAuditLogChange(change: JsonNode): AuditLogChange =
  ...
  of "type":
    if change.hasKey("new_value"):
      ...
      of JString:
        result.new_value = newAuditLogChangeValue(change[
                "new_value"].str)
      of JInt:
        result.new_value = newAuditLogChangeValue(change[
                "new_value"].num)
    ...
    if change.hasKey("old_value"):
      ...
      of JString:
        result.old_value = newAuditLogChangeValue(change[
                "old_value"].str)
      of JInt:
        result.old_value = newAuditLogChangeValue(change[
                "old_value"].num)
...
proc newAuditLog*(node: JsonNode): AuditLog {.inline.} =
  result = AuditLog(
      ...
      audit_log_entries: newSeq[AuditLogEntry](node[
              "audit_log_entries"].elems.len)
  )
...
proc getGuildMember*(c: Cache, guild, memberid: string): tuple[
        member: GuildMember, exists: bool] {.gcsafe.} =
...
proc getRole*(c: Cache, guildid, roleid: string): tuple[role: Role,
        exists: bool] {.gcsafe.} =
