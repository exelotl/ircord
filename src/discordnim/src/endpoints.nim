...
proc endpointAttachment*(cid, aid, fname: string): string {.
        inline.} = CDN_ATTACHMENTS & cid & "/" & aid & "/" & fname & ".png"

proc endpointAvatar*(uid, hash: string): string {.inline.} = CDN_AVATARS & uid &
        "/" & hash & ".png"

proc endpointAvatarAnimated*(uid, hash: string): string {.
        inline.} = CDN_AVATARS & uid & "/" & hash & ".gif"

proc endpointGuildIcon*(gid, hash: string): string {.inline.} = CDN_ICONS &
        gid & "/" & hash & ".png"

proc endpointGuildSplash*(gid, hash: string): string {.inline.} = CDN_SPLASHES &
        gid & "/" & hash & ".png"

proc endpointGroupIcon*(cid, hash: string): string {.
        inline.} = CDN_CHANNEL_ICONS & cid & "/" & hash & ".png"
...
proc endpointGuildChannels*(gid: string): string {.inline.} = endpointGuild(
        gid) & "/channels"

proc endpointGuildMembers*(gid: string): string {.inline.} = endpointGuild(
        gid) & "/members"

proc endpointGuildMember*(gid, uid: string): string {.
        inline.} = endpointGuildMembers(gid) & "/" & uid

proc endpointEditNick*(gid: string): string {.inline.} = endpointGuildMembers(
        gid) & "/@me/nick"

proc endpointGuildMemberRoles*(gid, uid, rid: string): string {.
        inline.} = endpointGuildMember(gid, uid) & "/roles/" & rid

proc endpointGuildBans*(gid: string): string {.inline.} = endpointGuild(gid) & "/bans"

proc endpointGuildBan*(gid, uid: string): string {.inline.} = endpointGuildBans(
        gid) & "/" & uid

proc endpointGuildRoles*(gid: string): string {.inline.} = endpointGuild(gid) & "/roles"

proc endpointGuildRole*(gid, rid: string): string {.
        inline.} = endpointGuildRoles(gid) & "/" & rid

proc endpointGuildPruneCount*(gid: string): string {.inline.} = endpointGuild(
        gid) & "/prune"

proc endpointGuildVoiceRegions*(gid: string): string {.inline.} = endpointGuild(
        gid) & "/regions"

proc endpointGuildInvites*(gid: string): string {.inline.} = endpointGuild(
        gid) & "/invites"

proc endpointGuildIntegrations*(gid: string): string {.inline.} = endpointGuild(
        gid) & "/integrations"

proc endpointGuildIntegration*(gid, iid: string): string {.
        inline.} = endpointGuildIntegrations(gid) & "/" & iid

proc endpointSyncGuildIntegration*(gid, iid: string): string {.
        inline.} = endpointGuildIntegration(gid, iid) & "/sync"

proc endpointGuildEmbed*(gid: string): string {.inline.} = endpointGuild(gid) & "/embed"

proc endpointGuildAuditLog(gid: string): string {.inline.} = endpointGuild(
        gid) & "/audit-logs"

proc endpointGuildEmojis(gid: string): string {.inline.} = endpointGuild(gid) & "/emojis"

proc endpointGuildEmoji(gid, eid: string): string {.
        inline.} = endpointGuildEmojis(gid) & "/" & eid
...
proc endpointLeaveGuild*(gid: string): string {.
        inline.} = endpointCurrentUserGuilds() & "/" & gid
